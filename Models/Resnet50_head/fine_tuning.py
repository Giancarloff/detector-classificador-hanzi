...
for name, param in resnet.named_parameters():
    if "layer4" in name or "fc" in name:  # Descongela a última camada e o head
...
optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, resnet.parameters()), lr=LR) # Adicionado weight_decay (ver ponto 4)
...
if __name__ == "__main__":
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if avg_loss < best_loss:
            best_loss = avg_loss
            torch.save(model.state_dict(), f'Models/Resnet50_head/{MODEL_NAME}.pth')
            print(f"Modelo salvo em {MODEL_NAME} com loss: {best_loss:.4f}")

        print(f"Epoch {epoch+1}/{NUM_EPOCHS}, Loss: {avg_loss:.8f}, Val Acc: {val_acc:.2f}%, LR: {scheduler.get_last_lr()[0]:.6f}")
